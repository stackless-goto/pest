...
//
// Copyright (c) 2020 mb <mb@64k.by>
// Copyright (c) 2017-18 Melissa E. O'Neill

#pragma once

/*
a cxx implementation of a family of {xoshiro} generators.

Based on the design and public domain implementation of Xoshiro by written in 2018 by David Blackman
and Sebastiano Vigna.  It is a variation on George Marsaglia's for XorShift PRNG scheme and the same
(G)LFSR theory applies.

Xoshiro256** fails PractRand if passed through certain bijections (e.g., multiplication by 57) and
the entire family suffers from issues due to near repeats but these near-repeat issues are unlikely
to be encountered in normal-sized variants without deliberately contriving a PRNG state that
generates them.
...
class xoshiro_x4 {
 protected:
  ...
  static inline itype rotl( itype const x, unsigned const k ) noexcept {
    return static_cast<itype>( ( x << k ) | ( x >> ( ITYPE_BITS - k ) ) );
  }
 ...
 public:
  ...
  void advance() {
    const uint64_t t = static_cast<uint64_t>( s1_ ) << a;
...
using xoshiro512plus64v1_0 = detail::xoshiro_plus<detail::xoshiro_x8<uint64_t, uint64_t, 11, 21>>;

using xoshiro512plus64 = xoshiro512plus64v1_0;

using xoshiro512starstar64v1_0 =
    detail::xoshiro_starstar<detail::xoshiro_x8<uint64_t, uint64_t, 11, 21>, 5, 7, 9>;

using xoshiro512starstar64 = xoshiro512starstar64v1_0;

// - 256 state bits, uint64_t output, period 2^256 - 1

using xoshiro256plus64v1_0 = detail::xoshiro_plus<detail::xoshiro_x4<uint64_t, uint64_t, 17, 45>>;

using xoshiro256plus64 = xoshiro256plus64v1_0;

using xoshiro256starstar64v1_0 =
    detail::xoshiro_starstar<detail::xoshiro_x4<uint64_t, uint64_t, 17, 45>, 5, 7, 9>;

using xoshiro256starstar64 = xoshiro256starstar64v1_0;

// - 128 state bits, uint32_t output, period 2^128 - 1

using xoshiro128plus32v1_0 = detail::xoshiro_plus<detail::xoshiro_x4<uint32_t, uint32_t, 9, 11>>;

using xoshiro128plus32 = xoshiro128plus32v1_0;

using xoshiro128starstar32v1_0 =
    detail::xoshiro_starstar<detail::xoshiro_x4<uint32_t, uint32_t, 9, 11>, 5, 7, 9>;
...
using xoshiro64plus16xxx = detail::xoshiro_plus<detail::xoshiro_x4<uint16_t, uint16_t, 5, 11>>;

using xoshiro64plus16 = xoshiro64plus16xxx;

using xoshiro64starstar16xxx =
    detail::xoshiro_starstar<detail::xoshiro_x4<uint16_t, uint16_t, 5, 11>, 5, 7, 9>;

using xoshiro64starstar16 = xoshiro64starstar16xxx;

// - 32 state bits, uint8_t output, period 2^32 - 1

using xoshiro32plus8xxx = detail::xoshiro_plus<detail::xoshiro_x4<uint8_t, uint8_t, 3, 7>>;

using xoshiro32plus8yyy = detail::xoshiro_plus<detail::xoshiro_x4<uint8_t, uint8_t, 3, 1>>;

using xoshiro32plus8 = xoshiro32plus8xxx;

using xoshiro32starstar8xxx =
    detail::xoshiro_starstar<detail::xoshiro_x4<uint8_t, uint8_t, 3, 7>, 5, 7, 9>;

using xoshiro32starstar8yyy =
    detail::xoshiro_starstar<detail::xoshiro_x4<uint8_t, uint8_t, 3, 1>, 5, 7, 9>;
